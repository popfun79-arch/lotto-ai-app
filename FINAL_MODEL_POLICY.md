...
## Historical Validation Ledger 운영 원칙 v4.5

1. 각 목표 회차는 직전 200회만 사용한다.
2. 목표 회차와 이후 데이터는 학습에 포함하지 않는다.
3. 과거 100회는 진단 기준선으로 사용한다.
4. 이전 50회와 최근 50회를 비교하여 Regime 변화를 본다.
5. 과거 100회에 맞춰 가중치를 반복 수정하지 않는다.
6. 실패 원인은 파이프라인 단계별로 기록한다.
   - 후보번호 단계
   - 번호합 상태
   - GAP합 상태
   - 조합 구성/포트폴리오
   - Pattern Master 상위권 포착
7. Top of the Best Final Pattern은 결정론적이므로 Seed 직접 실패 원인으로
   분류하지 않는다. Seed 민감도는 별도 GA/랜덤 탐색 검증으로 관리한다.
8. v4.5 이후 신규 실제 회차는 동일 지표로 계속 누적한다.
